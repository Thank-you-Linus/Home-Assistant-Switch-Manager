...
SHAPE_PATH_SCHEMA = vol.Schema({vol.Required("d"): cv.string})
...
SWITCH_MANAGER_CONFIG_ACTION_SCHEMA = vol.Schema(
    {
        ...
        vol.Required(
            "sequence", default=[]
        ): cv.ensure_list,  # cv.SCRIPT_SCHEMA: This was causing problems and not parsing json format when action delay etc was used
    }
)
SWITCH_MANAGER_CONFIG_BUTTON_SCHEMA = vol.Schema(
    {
        vol.Required("actions"): vol.All(
            cv.ensure_list, [SWITCH_MANAGER_CONFIG_ACTION_SCHEMA]
        )
    }
)
SWITCH_MANAGER_CONFIG_SCHEMA = vol.Schema(
    {
        ...
        vol.Required("buttons"): vol.All(
            cv.ensure_list, [SWITCH_MANAGER_CONFIG_BUTTON_SCHEMA]
        ),
    },
